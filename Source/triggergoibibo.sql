-- Create trigger in goibibo
DELIMITER //

CREATE TRIGGER db1_table1_trigger AFTER INSERT ON goibibo.price
FOR EACH ROW
BEGIN
...
-- Create trigger in db1 for AFTER UPDATE
DELIMITER //
CREATE TRIGGER db1_table1_update_trigger AFTER UPDATE ON goibibo.price
FOR EACH ROW
BEGIN
    UPDATE merged.price
...
-- Create trigger in db1 for AFTER DELETE
DELIMITER //

CREATE TRIGGER db1_table1_delete_trigger AFTER DELETE ON goibibo.price
FOR EACH ROW
BEGIN
